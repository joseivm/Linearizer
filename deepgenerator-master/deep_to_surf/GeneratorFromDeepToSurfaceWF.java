...
public class GeneratorFromDeepToSurfaceWF {
	...
	public GeneratorFromDeepToSurfaceWF(String pathSurface, String pathDeep, String pathTestDeep){
		...
		ArrayList <CoNLLHash> deepTreebank = CoNLLTreeConstructor.storeTreebank(pathDeep); // Reads from an input stream and adds each word to a CoNLLHash dictionary
		listPosDeep = this.obtainKeys(deepTreebank); // The keys of the CoNLLHash list?
		mbd = new ModelBubbleDetection(listPosDeep); // makes a ModelBubbleDetection object, which maps each word in the input list to two BufferedWriters
				...
				//ArrayList<String> surfaceIds=surfaceSentence.getIds();
					...
					while(itDeepIds.hasNext()){
						...
						if (bubbleArray.size()>1){
							...
							String linksString=mbia.getSVMiAttachmentsSingleFile("iattachment_output_"+mapping.get(bubble)+".svm");
							...
							stLinks=new StringTokenizer(linksString,",");
					...
					while(itDeepIds.hasNext()){
						...
						/*if (featsDeep.contains("spos")){
							String spos=CoNLLHash.getSubFeat(deepSentence.getFEAT(id),"spos");
							featsSurface+="spos="+spos+"|";
						}
						
						if (featsDeep.contains("voice")){
							String voice=CoNLLHash.getSubFeat(deepSentence.getFEAT(id),"voice");
							featsSurface+="voice="+voice+"|";
						}
						
						if (featsDeep.contains("aux_rel")){
							String aux_rel=CoNLLHash.getSubFeat(deepSentence.getFEAT(id),"aux_rel");
							featsSurface+="aux_rel="+aux_rel+"|";
						}*/
						...
						while(childIt.hasNext()){
							...
							if (!bubble.equals("[]") && mapping.get(bubble)!=null){
								...
								String linksString=mbea.getSVMeAttachmentsSingleFile("eAttachment_output_"+mapping.get(bubble)+".svm");
								//System.out.println(linksString);
					...
					//POST-PROCESSING- idiosyncratic rules for SPANISH.
					Set<String> idsSurf=surfIdToWords.keySet();
					ArrayList<String> newConlllines=new ArrayList<String>();
					for (int i=1;i<idsSurf.size()+1;i++){
						String idS=i+"";
						if (idsSurf.contains(idS)) {
							String feat=surfIdToFeats.get(idS);
							String head=surfIdToHead.get(idS);
							String word=surfIdToWords.get(idS);
							String pos=surfIdToPos.get(idS);
							...
							String deepFeat=null;
							String deepLemma=null;
							String deepWord=null;
							String bubbleGovernor=null;
							if (surfIdToDeepIds.get(idS)!=null) {
								deepFeat=deepSentence.getFEAT(surfIdToDeepIds.get(idS));
								deepLemma=deepSentence.getLemma(surfIdToDeepIds.get(idS));
								deepWord=deepSentence.getForm(surfIdToDeepIds.get(idS));
								bubbleGovernor=deepIdToBubbleGovernor.get(surfIdToDeepIds.get(idS));
								if (word.startsWith("pers") && word.contains("_")){
									surfIdToWords.put(idS, "su");
									surfIdToDeprel.put(idS, "det_31");
								}
								if (deepFeat.contains("spos=relative_pronoun")){
									surfIdToWords.put(idS, "que");
								}
								
								if (deepLemma.equals("si_mismo") && deepFeat.contains("coref_id")){
									surfIdToWords.put(idS, "se");
									surfIdToDeprel.put(idS, "aux_refl_31");
								}

								if (deepFeat.contains("voice=PASS") && !(deepFeat.contains("analyt_pass"))){// && idS.equals(bubbleGovernor)){
									String newconllLine=idsSurf.size()+1+"\tse\tse\tse\tPP\tPP\t"+"_"+"\t"+"_"+"\t"+idS+"\t"+idS+"\t"+"aux_refl_31"+"\t"+"aux_refl_31"+"\t"+"_"+"\t"+"_"+"\n";
									//System.out.println(newconllLine);
									newConlllines.add(newconllLine);
								}
							}

							if (deprel.startsWith("analyt_pass") || deprel.startsWith("analyt_perf")){
								feat+="|finiteness=PART";
							}
							if (deprel.startsWith("analyt_progr")){
								feat+="|finiteness=GER";
							}
...
//							String conll10Line=idS+"\t"+surfIdToWords.get(idS)+"\t"+surfIdToWords.get(idS)+"\t"+surfIdToPos.get(idS)+"\t"+surfIdToPos.get(idS)+"\t"+"\t"+feat+"\t"+"_"+"\t"+head+"\t"+"_"+"\t"+deprel+"\t"+deprel+"\t"+"_"+"\t"+"_"+"\n";
					...
					for (int i=0;i<newConlllines.size();i++){
						bw.write(newConlllines.get(i));
					}
...
public void generateBubblesAndLemmasForTraining(){
	...
	ArrayList <CoNLLHash> surfaceTreebank = CoNLLTreeConstructor.storeTreebank(pathSurface); // Reads from an input stream and adds each word to a CoNLLHash dictionary
	ArrayList <CoNLLHash> deepTreebank = CoNLLTreeConstructor.storeTreebank(pathDeep);
	...
	while(itSurfTreebank.hasNext() && itDeepTreebank.hasNext()){
		...
		while(itDeepIds.hasNext()){
			...
			while(idxFound){
				String idx = "id"+cont;
				String feats = deepSentence.getFEAT(id);
				String surfNode = CoNLLHash.getSubFeat(feats, idx); // surfNode is a string that has the "idx" subfeature of deepSentence
				if (!surfNodes.contains(surfNode)){
					
						// System.out.println(idx);
						if (!surfNode.equals("")) {
							// System.out.println(surfNode);
							if (!surfNode.contains("_")
									&& !surfNode.contains("bis")
									&& !surfNode.equals("")) { // Here, surfNode is used as an ID to get the string "pos" 
								 ...
								 * pos.equals("VV")){
								 * System.out.println(sentenceCounter+"--->"+id
								 * +
								 * ": "+deepSentence.getFEAT(id)+"---"+deepSentence
								 * .getLemma(id)+" idSuperficie:"+surfNode); }
								 */
								if (!pos.contains("(deep)")) {
									ArrayList<String> listLemma = posToWord
											.get(pos);
									if (listLemma == null)
										listLemma = new ArrayList<String>();
									if (!surfNode.equals("1")) {
										listLemma = addLemma(
												surfaceSentence.getLemma(surfNode),listLemma); // this appends the lemma that corresponds to surfNode in surfaceSentence to listLemma
															
									} else {
										listLemma = addLemma(surfaceSentence.getLemma(surfNode).toLowerCase(), listLemma); // this appends the lemma that corresponds to surfNode in surfaceSentence to listLemma
																			
									}
									// adds surfNode and a bunch of its features
									// to several dictionaries
									if (!pos.equals("_")) {
			...
			// adds the current bubble to bubbles2Mores if it has more than 1 element and it is not already in bubbles2More
			if (!bubbles2More.contains(bubble.toString()) && bubble.size()>1) {
				bubbles2More.add(bubble.toString());
			}
			Iterator<String> itLemma = idToLemma.keySet().iterator();
			// iterates through the IDs, for each ID it adds its corresponding lemma and features to a document that will be used to train the svm
			while(itLemma.hasNext()){
				String idKey = itLemma.next();
			...
			mbd.addLine(bubble.toString(), id, deepSentence, true, posDeep);
				...
				// Adds bubble to oldBubble if its not there, why does it iterate?
				ArrayList <ArrayList <String>> oldBubble = hsBubbles.get(posDeep);
				if (oldBubble!=null) {
					for (int i=0;i<oldBubble.size();i++){
						if (!oldBubble.contains(bubble)){
			...
			//----------------------------------------------------------------------------------------------
				...
				if (bubble.size()>1) {
					...
					// This part iterates through the nodes in the surface sentence. For each node, it checks 
					// whether that pos is in contPos. This seems to count the number of times a certain pos occurs
					// in a sentence.
					int k = 0;
					ArrayList <String> links = new ArrayList <String>();
					
					int counter = 0;
					HashMap <String, String> correspondences = new HashMap <String, String>(); // maps IDs to their pos
					...
					// Iterates through all the surfNodes, finds their pos, then uses that and other information to 
					// assign dependency relations between words. These dependency relations are stored in the links list.
				...
				// Iterates through the links and removes the arrows from them, and adds both components to the 
				// numberedBubble list.
				...
				// Iterates through the links list and adds all the words that are pointed to to the internals list.
				while(st.hasMoreTokens()){
				...
				// Iterates through all the words in a sentence, and whichever word is not pointed to in a dependency relation
				// is made the governor of that bubble.
				while(st2.hasMoreTokens()){
					String posS = st2.nextToken(",");
